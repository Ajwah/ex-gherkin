defmodule ExGherkin.AstTraverser do
  ...
  defmacro __using__(reducer) do
    quote location: :keep do
      ...
      def background_constituents(
            {:background = token, {:meta, meta, {:type, :multiples}},
      ...
      def rule_constituents(
        ...
        {token, Acc.local(acc_title), ctx, Acc.local(acc_description_block),
         Acc.local(acc_background), Acc.local(acc_scenario_blocks)}
      ...
      def examples_constituents(
        ...
        {Acc.local(acc_title), Acc.local(acc_ctx), Acc.local(acc_tags),
         Acc.local(acc_description_block), Acc.local(acc_data_table)}
      ...
      def handle_scenario(
            {:scenario = token, {:meta, meta, {:type, :multiples}},
             {:constituents, constituents}},
            acc = Acc.storage()
          ) do
