defmodule ParserTest do
  ...
  def parse(a, ast_standard_path_to_use \\ :use_absolute_path)

  def parse([path: path], ast_standard_path_to_use) do
    path_to_use =
      if ast_standard_path_to_use == :use_absolute_path do
        path
      else
        [relative_path] = Regex.run(~r/testdata.*/, path)
        relative_path
      end
    ...
    |> ExGherkin.prepare()
    |> ExGherkin.run()
    |> ParserSupport.to_ast_standard(path_to_use)
    |> Jason.encode!()
  ...
  describe "#run works for all Full Feature Files" do
    __DIR__
